...
@HiltViewModel
class MinifiguresSearchViewModel @Inject constructor (
    ...
    private val _minifiguresPagingData = MutableStateFlow<PagingData<MinifigureWithSeriesName>>(PagingData.empty())
    val minifiguresPagingData = _minifiguresPagingData.asStateFlow()
    ...
    private var pager: Pager<Int, MinifigureWithSeriesName>? = null

    private var getMinifiguresPagingDataJob: Job? = null

    init {
        // On process recreation, due to system-initiated process death, get the minifigures paging
        // data corresponding to the restored search text field value.
        if (searchTextFieldValue.text.isNotEmpty()) {
            createPagerAndStartJobToGetMinifiguresPagingData(searchTextFieldValue.text.trimEnd())
        }
    }
    ...
    fun changeSearchTextFieldValueAndMinifiguresPagingData(textFieldValue: TextFieldValue) {
        ...
        if (searchTextFieldValue.text.isNotEmpty()) {
            ...
            getMinifiguresPagingDataJob?.cancel()
            pager = null
            _minifiguresPagingData.value = PagingData.empty()
        }
    }

    fun clearText() {
        changeSearchTextFieldValueAndMinifiguresPagingData(TextFieldValue(""))
    }

    private fun createPagerAndStartJobToGetMinifiguresPagingData(query: String) {
        getMinifiguresPagingDataJob?.cancel()

        pager = Pager(
            config = pagingConfig,
            pagingSourceFactory = { repo.getMinifiguresMatchingQuery(query) }
        )

        getMinifiguresPagingDataJob = viewModelScope.launch {
            pager!!.flow.cachedIn(viewModelScope).flowOn(Dispatchers.IO).collect { minifiguresPagingData ->
                _minifiguresPagingData.value = minifiguresPagingData
            }
        }
    }
